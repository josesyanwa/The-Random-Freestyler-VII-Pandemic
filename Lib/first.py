...
def run_trading_script():
    ...
    for index, row in current_df.iterrows():
        ...
        if trade_signal == 'Sell':
            ...
            high = df[df['symbol'] == symbol]['high'].iloc[-1]
            sl = high + 20 * point
            price_diff = abs(price - sl)

            if price_diff <= 50 * point:
                lot = 0.2
            elif 50 * point < price_diff <= 100 * point:
                lot = 0.18
            elif 100 * point < price_diff <= 150 * point:
                lot = 0.16
            elif 150 * point < price_diff <= 200 * point:
                lot = 0.14
            elif 200 * point < price_diff <= 250 * point:
                lot = 0.12
            elif 250 * point < price_diff <= 300 * point:
                lot = 0.1
            elif 300 * point < price_diff <= 350 * point:
                lot = 0.08
            elif 350 * point < price_diff <= 400 * point:
                lot = 0.06
            elif 400 * point < price_diff <= 450 * point:
                lot = 0.04
            else:
                print(f"R to R 500:{price_diff/point} for {symbol} is less than 1:1, skipping trade")
                continue
            ...
            low = df[df['symbol'] == symbol]['low'].iloc[-1]
            sl = low - 20 * point
            price_diff = abs(price - sl)
...
if __name__ == "__main__":
    ...
    schedule.every().hour.at("01:56").do(run_trading_script)
    schedule.every().hour.at("03:56").do(run_trading_script)
    schedule.every().hour.at("05:56").do(run_trading_script)
    schedule.every().hour.at("07:56").do(run_trading_script)
    schedule.every().hour.at("09:56").do(run_trading_script)
    schedule.every().hour.at("11:56").do(run_trading_script)
    schedule.every().hour.at("13:56").do(run_trading_script)
    schedule.every().hour.at("15:56").do(run_trading_script)
    schedule.every().hour.at("17:56").do(run_trading_script)
    schedule.every().hour.at("19:56").do(run_trading_script)
    schedule.every().hour.at("21:56").do(run_trading_script)
    schedule.every().hour.at("23:56").do(run_trading_script)
    schedule.every().hour.at("25:56").do(run_trading_script)
    schedule.every().hour.at("27:56").do(run_trading_script)
    schedule.every().hour.at("29:56").do(run_trading_script)
    schedule.every().hour.at("31:56").do(run_trading_script)
    schedule.every().hour.at("33:56").do(run_trading_script)
    schedule.every().hour.at("35:56").do(run_trading_script)
    schedule.every().hour.at("37:56").do(run_trading_script)
    schedule.every().hour.at("39:56").do(run_trading_script)
    schedule.every().hour.at("41:56").do(run_trading_script)
    schedule.every().hour.at("43:56").do(run_trading_script)
    schedule.every().hour.at("45:56").do(run_trading_script)
    schedule.every().hour.at("47:56").do(run_trading_script)
    schedule.every().hour.at("49:56").do(run_trading_script)
    schedule.every().hour.at("51:56").do(run_trading_script)
    schedule.every().hour.at("53:56").do(run_trading_script)
    schedule.every().hour.at("55:56").do(run_trading_script)
    schedule.every().hour.at("57:56").do(run_trading_script)
